...
bool SettingsStruct_tmpl<N_TASKS>::getSPI_pins(int8_t spi_gpios[3]) const {
  ...
  if (isSPI_valid()) {
    ...
    switch (SPI_selection) {
      ...
      {
        spi_gpios[0] = VSPI_SCLK;
        spi_gpios[1] = VSPI_MISO;
        spi_gpios[2] = VSPI_MOSI;
        break;
      }
...
bool SettingsStruct_tmpl<N_TASKS>::isSPI_pin(int8_t pin) const {
  if ((pin < 0) || (isTaskDeviceUsePin(pin))) return false;
  int8_t spi_gpios[3];
...
//             (SPI_MISO_pin == -1) ||
             ...
             (isTaskDeviceUsePin(SPI_SCLK_pin)) ||
             (isTaskDeviceUsePin(SPI_MISO_pin)) ||
             (isTaskDeviceUsePin(SPI_MOSI_pin)));
#ifndef CONFIG_IDF_TARGET_ESP32C3
  if (InitSPI == static_cast<uint8_t>(SPI_Options_e::Vspi))
    return !((isTaskDeviceUsePin(VSPI_SCLK)) ||
             (isTaskDeviceUsePin(VSPI_MISO)) ||
             (isTaskDeviceUsePin(VSPI_MOSI)));
#endif // ifndef CONFIG_IDF_TARGET_ESP32C3
  if (InitSPI == static_cast<uint8_t>(SPI_Options_e::Hspi))
    return !((isTaskDeviceUsePin(HSPI_SCLK)) ||
             (isTaskDeviceUsePin(HSPI_MISO)) ||
             (isTaskDeviceUsePin(HSPI_MOSI)));
  return true;
}
...
bool SettingsStruct_tmpl<N_TASKS>::isI2C_pin(int8_t pin) const {
  if ((pin < 0) || (isTaskDeviceUsePin(pin))) return false;
  return ((!isTaskDeviceUsePin(pin)) && (Pin_i2c_sda == pin ||  Pin_i2c_scl == pin));
}
...
template<unsigned int N_TASKS>
bool SettingsStruct_tmpl<N_TASKS>::isLED_pin(int8_t pin) const {
  if ((pin < 0) || (isTaskDeviceUsePin(pin))) return false;
  return Pin_status_led == pin;
}

template<unsigned int N_TASKS>
bool SettingsStruct_tmpl<N_TASKS>::isReset_pin(int8_t pin) const {
  if ((pin < 0) || (isTaskDeviceUsePin(pin))) return false;
  return Pin_Reset == pin;
}

template<unsigned int N_TASKS>
bool SettingsStruct_tmpl<N_TASKS>::isTaskDeviceUsePin(int8_t pin) const {
  if (pin < 0) return false;
  for (taskIndex_t x = 0; x < TASKS_MAX; x++)
  {
    if (((TaskDevicePin1[x] != -1) && (TaskDevicePin1[x] == pin)) ||
        ((TaskDevicePin2[x] != -1) && (TaskDevicePin2[x] == pin)) ||
        ((TaskDevicePin3[x] != -1) && (TaskDevicePin3[x] == pin)))
    {
      return true;
    }
  }
...
template<unsigned int N_TASKS>
taskIndex_t SettingsStruct_tmpl<N_TASKS>::isUsePinToTask(int8_t pin) const {
  if (pin < 0) return false;
  for (taskIndex_t x = 0; x < TASKS_MAX; x++)
  {
    if (((TaskDevicePin1[x] != -1) && (TaskDevicePin1[x] == pin)) ||
