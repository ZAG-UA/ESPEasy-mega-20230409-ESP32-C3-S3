...
//  SettingsStruct_tmpl() = default;
  ...
  // Return true when pin is one of the configured LED pins.
  bool isLED_pin(int8_t pin) const;

  // Return true when pin is one of the configured Reset pins.
  bool isReset_pin(int8_t pin) const;

  // Returns true if the output is busy in some task
  bool isTaskDeviceUsePin(int8_t pin) const;

  // Returns the number of the task using this pin
  taskIndex_t isUsePinToTask(int8_t pin) const;
