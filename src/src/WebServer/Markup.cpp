...
void addPinSelector_Item(PinSelectPurpose purpose, const String& gpio_label, int gpio, bool selected, bool disabled, const String& attr)
{
  if (gpio != -1) // empty selection can never be disabled...
  {
    ...
    if (getGpioInfo(gpio, pinnr, input, output, warning)) {
      ...
      bool includeLED = true;
      bool includeReset = true;
      bool includeUseTask = true;
#if FEATURE_ETHERNET
      bool includeEthernet = true;
#endif // if FEATURE_ETHERNET

      switch (purpose) {
        case PinSelectPurpose::LED:
          includeLED = false;
          if (!output) {
            return;
          }
          break;
        case PinSelectPurpose::Reset:
          includeReset = false;
          if (!input) {
            return;
          }
        ...
        case PinSelectPurpose::SPI_MOSI:
          includeSPI = false;
          if (purpose == PinSelectPurpose::SPI && !output) {
            return;
          }
          if (purpose == PinSelectPurpose::SPI_MISO && !input) {
            return;
          }
          if (purpose == PinSelectPurpose::SPI_MOSI && !output) {
            return;
          }
          break;
      ...
      if (includeLED && Settings.isLED_pin(gpio)) {
        disabled = true;
      }

      if (includeReset && Settings.isReset_pin(gpio)) {
        disabled = true;
      }
      ...
      if (!selected && includeUseTask && Settings.isTaskDeviceUsePin(gpio) && (Settings.TaskDeviceEnabled[Settings.isUsePinToTask(gpio)])) {
        disabled = true;
      }
...
void addPinSelect(PinSelectPurpose purpose, const String& id,  int choice)
{
  ...
  while (gpio <= MAX_GPIO) {
    ...
    if (UsableGPIO || (i == 0)) {
      ...
      String conflict = getConflictingUse_wrapped(gpio, purpose);
      gpio_label += (!conflict.isEmpty()) ? conflict : ((Settings.isTaskDeviceUsePin(gpio)) && (choice != gpio)) ? ((Settings.TaskDeviceEnabled[Settings.isUsePinToTask(gpio)]) ? F(" [Used by Task]") : F(" [Used by Task, not Enabled]")) : F("");
      addPinSelector_Item(
        purpose,
        gpio_label,
        gpio,
...
void addADC_PinSelect(AdcPinSelectPurpose purpose, const String& id,  int choice)
{
  ...
  if ((purpose == AdcPinSelectPurpose::ADC_Touch_TemperatureSensor) ||
      ...
      if (getADC_gpio_info(gpio, adc, ch, t)) {
        if ((purpose != AdcPinSelectPurpose::TouchOnly) || (t >= 0)) {
          ...
          if (adc != 0) {
            ...
            String conflict = getConflictingUse_wrapped(gpio);
            gpio_label += (!conflict.isEmpty()) ? conflict : ((Settings.isTaskDeviceUsePin(gpio)) && (choice != gpio)) ? ((Settings.TaskDeviceEnabled[Settings.isUsePinToTask(gpio)]) ? F(" [Used by Task]") : F(" [Used by Task, not Enabled]")) : F("");
          }
          addPinSelector_Item(
            PinSelectPurpose::Generic,
