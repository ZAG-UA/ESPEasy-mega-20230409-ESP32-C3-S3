...
void addFormPinStateSelect(int gpio, int choice)
{
  ...
  if (Settings.isI2C_pin(gpio)) {
    // do not add the pin state select for I2C pins
    enabled = false;
  }

  if (Settings.isSPI_pin(gpio)) {
    // do not add the pin state select for SPI pins
    enabled = false;
  }

  if (Settings.isLED_pin(gpio)) {
    // do not add the pin state select for Status LED pin
    enabled = false;
  }

  if (Settings.isReset_pin(gpio)) {
    // do not add the pin state select for Reset pin
    enabled = false;
  }
  ...
  if (getGpioInfo(gpio, pinnr, input, output, warning)) {
    ...
    addSelector(id, nr_options, options, option_val, nullptr, choice, false, enabled);
    {
      ...
      if (!conflict.isEmpty()) {
        addUnit(conflict);
      } else if (Settings.isTaskDeviceUsePin(gpio)) {
        addUnit((Settings.TaskDeviceEnabled[Settings.isUsePinToTask(gpio)]) ? F("Used by Task") : F("Used by Task, not Enabled"));
      }
    }
  }
}
