...
void handle_hardware() {
  ...
  if (isFormItem(F("pled"))) {
    ...
    Settings.I2C_Multiplexer_ResetPin = getFormItemInt(F("pi2cmuxreset"));
      ...
      if (Settings.InitSPI == static_cast<int>(SPI_Options_e::Vspi)) { // VSPI SPI GPIO pins
        if (!Settings.isSPI_valid()) { // Checks
#if CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32S3
          error += F("One of the pins of the SPI3 bus is occupied by another task!\n");
#else // if CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32S3
          error += F("One of the pins of the VSPI bus is occupied by another task!\n");
#endif // if CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32S3
        }
      }
#endif // ifndef CONFIG_IDF_TARGET_ESP32C3
      if (Settings.InitSPI == static_cast<int>(SPI_Options_e::Hspi)) { // HSPI SPI GPIO pins
        if (!Settings.isSPI_valid()) { // Checks
#if CONFIG_IDF_TARGET_ESP32C3 || CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32S3
          error += F("One of the pins of the FSPI bus is occupied by another task!\n");
#else // if CONFIG_IDF_TARGET_ESP32C3 || CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32S3
          error += F("One of the pins of the HSPI bus is occupied by another task!\n");
  ...
  addFormPinSelect(PinSelectPurpose::LED, formatGpioName_output(F("LED")), F("pled"), Settings.Pin_status_led);
  addFormCheckBox(F("Inversed LED"), F("pledi"), Settings.Pin_status_led_Inversed);
#ifdef ESP32
  addFormNote(F("Use with &rsquo;Inversed&rsquo; checked for onboard LED"));
#else
  addFormNote(F("Use &rsquo;GPIO-2 (D4)&rsquo; with &rsquo;Inversed&rsquo; checked for onboard LED"));
#endif
  addFormSubHeader(F("Reset Pin"));
  addFormPinSelect(PinSelectPurpose::Reset, formatGpioName_input(F("Switch")), F("pres"), Settings.Pin_Reset);
  ...
  #else // ifndef CONFIG_IDF_TARGET_ESP32C3
    addFormSelector_script(F("Init SPI"), F("initspi"), 3, spi_options, spi_index, nullptr, Settings.InitSPI, F("spiOptionChanged(this)"));
  #endif // ifndef CONFIG_IDF_TARGET_ESP32C3

    // User-defined pins
    addFormPinSelect(PinSelectPurpose::SPI, formatGpioName_output(F("CLK")),  F("spipinsclk"), Settings.SPI_SCLK_pin);
    addFormPinSelect(PinSelectPurpose::SPI_MOSI, formatGpioName_output(F("MOSI")), F("spipinmosi"), Settings.SPI_MOSI_pin);
    addFormPinSelect(PinSelectPurpose::SPI_MISO, formatGpioName_input_optional(F("MISO")),  F("spipinmiso"), Settings.SPI_MISO_pin);
