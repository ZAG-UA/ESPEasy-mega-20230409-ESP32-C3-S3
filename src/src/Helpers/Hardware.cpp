...
#ifdef ESP32
  uint32_t lastI2CClockSpeed = 100000UL;
#endif // ifdef ESP32
#ifdef ESP8266
  static uint32_t lastI2CClockSpeed = 100000;
#endif // ifdef ESP8266

static int8_t last_sda = Settings.Pin_i2c_sda;
static int8_t last_scl = Settings.Pin_i2c_scl;

/********************************************************************************************\
 * Initialize specific hardware settings (only global ones, others are set through devices)
 \*********************************************************************************************/
void setPinBootState(int gpio)
{
  const bool serialPinConflict = (Settings.UseSerial && 
#ifdef ESP32
    ((gpio == TX_GPIO) || (gpio == RX_GPIO))
#else // ifdef ESP32
    ((gpio == 1) || (gpio == 3))
#endif // ifdef ESP32
  );
...
void hardwareInit()
{
  ...
  if (Settings.isSPI_valid())
  {
    ...
    switch (SPI_selection) {
      ...
      {
        SPI.begin(VSPI_SCLK, VSPI_MISO, VSPI_MOSI); // VSPI
        break;
      }
      ...
      {
        SPI.begin(Settings.SPI_SCLK_pin, Settings.SPI_MISO_pin, Settings.SPI_MOSI_pin); // User-defined SPI
        break;
      }
...
void I2CBegin(int8_t sda, int8_t scl, uint32_t clockFreq) {
...
#ifdef ESP32
  lastI2CClockSpeed = Wire.getClock();
#endif // ifdef ESP32
  ...
  if (sda != last_sda) {
    setPinBootState(last_sda);
    if (getGpioPullResistor(sda, hasPullUp, hasPullDown)) {
      if (hasPullUp) {
  ...
  if (scl != last_scl) {
    setPinBootState(last_scl);
    if (getGpioPullResistor(scl, hasPullUp, hasPullDown)) {
      if (hasPullUp) {
  ...
  if (getADC_gpio_info(pin, adc, ch, t)) {
    ...
    switch (adc) {
      case 0:
        ...
        value = (int)(temperatureRead());     //   value = hallRead();
        break;
      case 1:
        canread = true;
        break;
...
void readBootCause() {
  ...
  switch (rtc_get_reset_reason(0)) {
   ...
   #ifdef CONFIG_IDF_TARGET_ESP32
    case SW_CPU_RESET:
    case TGWDT_CPU_RESET:   lastBootCause = BOOT_CAUSE_SOFT_RESTART; break;  // Both call to ESP.reset() and on exception crash
   #endif // ifdef CONFIG_IDF_TARGET_ESP32
    ...
    case USB_UART_CHIP_RESET: break;
    case USB_JTAG_CHIP_RESET: break;
    case POWER_GLITCH_RESET:  lastBootCause = BOOT_CAUSE_POWER_UNSTABLE; break;
...
bool getGpioInfo(int gpio, int& pinnr, bool& input, bool& output, bool& warning) {
  ...
  if ((gpio == 0) || (gpio == 46)) {
    // GPIO 0  State during boot determines boot mode, GPIO 46 Part of the boot strapping pins.
    warning = true;
  }

#elif defined(CONFIG_IDF_TARGET_ESP32C3) // ifdef CONFIG_IDF_TARGET_ESP32S2
  ...
  if ((gpio > 11) && (gpio < 18)) {
    // SPIHD, SPIWP, SPICS0, SPICLK, SPIQ, SPID pins of ESP32-C3 and ESP32-C3FH4
    // are connected to embedded flash and not recommended for other uses.
    input  = false;
    output = false;
    warning = true;
  ...
  if ((gpio == 2) || (gpio == 8) || (gpio == 9)) {
    // GPIO 2 & 8 & 9 State during boot determines boot mode, Part of the boot strapping pins.
    warning = true;
  }

//  if ((warning == false) && (input == false) && (output == false)) {
  if ((input == false) && (output == false)) {
    return false;
  }

#elif defined(CONFIG_IDF_TARGET_ESP32S3) // ifdef CONFIG_IDF_TARGET_ESP32S2

  // Input GPIOs:  0-21, 26, 33-48
  // Output GPIOs: 0-21, 26, 33-48
  input  = gpio <= 48;
  output = gpio <= 48;
  ...
  if ((gpio == 0) || (gpio == 3) || (gpio == 46)) {
    // GPIO 0  State during boot determines boot mode, GPIO 46 Part of the boot strapping pins.
    warning = true;
  }

#else // ifdef CONFIG_IDF_TARGET_ESP32S2
...
bool getGpioPullResistor(int gpio, bool& _hasPullUp, bool& _hasPullDown) {
  _hasPullDown = false;
  _hasPullUp   = false;
...
#elif defined(CONFIG_IDF_TARGET_ESP32C3) // ifdef ESP32S2

  if ((gpio > 10) && (gpio < 18)) {
    // For GPIO 11 - 17, no pull-up nor pull-down.
  } else if (gpio == 2 || gpio == 8 || gpio == 9) {
    // No Pull-down on GPIO2,8,9
    // Just connect GPIO2,8,9 to VDD via a 10 kOhm resistor.
  } else if (gpio <= 21) {
    _hasPullUp   = true;
    _hasPullDown = true;
  }
...
#endif // ifdef CONFIG_IDF_TARGET_ESP32S2
  return true;
}
...
bool getADC_gpio_info(int gpio_pin, int& adc, int& ch, int& t)
{
  ...
  switch (gpio_pin) {
    case -1: adc = 0; break; // Temperature Sensor
...
#elif defined(CONFIG_IDF_TARGET_ESP32C3) // if defined(CONFIG_IDF_TARGET_ESP32S2) || defined(CONFIG_IDF_TARGET_ESP32S3)

  switch (gpio_pin) {
    case -1: adc = 0; break; // Temperature Sensor
    case 1:  adc = 1; ch = 0;  break;
    case 2:  adc = 1; ch = 1;  break;
    case 3:  adc = 1; ch = 2;  break;
    case 4:  adc = 1; ch = 3;  break;
    case 5:  adc = 1; ch = 4;  break;
    case 6:  adc = 2; ch = 0;  break;
    default:
      return false;
  }
  ...
  // Classic ESP32
  switch (gpio_pin) {
    case -1: adc = 0; break; // Temperature Sensor ; // Hall effect Sensor
...
void initAnalogWrite()
{
  ...
  for (uint8_t x = 0; x < LED_CHANNELS; x++) {
    ledChannelPin[x]  = -1;
...
int8_t ledChannelPin[LED_CHANNELS];
uint32_t ledChannelFreq[LED_CHANNELS] = { 0 };
...
int8_t attachLedChannel(int pin, uint32_t frequency)
{
  ...
  {
    for (uint8_t x = 0; x < LED_CHANNELS && ledChannel == -1; x++) { // find free channel
...
bool set_Gpio_PWM(int gpio, uint32_t dutyCycle, uint32_t fadeDuration_ms, uint32_t& frequency, uint32_t& key)
{
  ...
  if (fadeDuration_ms != 0)
  {
    ...
    // getPinState(PLUGIN_GPIO, gpio, &prev_mode, &prev_value);
  ...
  // setPinState(PLUGIN_GPIO, gpio, PIN_MODE_PWM, dutyCycle);
