...
String formatGpioName_ADC(int gpio_pin) {
  ...
  if (getADC_gpio_info(gpio_pin, adc, ch, t)) {
    if (adc == 0) {
      return F("Temperature Sensor");      //  return F("Hall Effect");
    }
...
const __FlashStringHelper* getConflictingUse(int gpio, PinSelectPurpose purpose)
{
  if (Settings.UseSerial) {
    ...
    if (gpio == TX_GPIO) { return F("USB D-"); }
    if (gpio == RX_GPIO) { return F("USB D+"); }
#else // if ARDUINO_USB_CDC_ON_BOOT
    if (gpio == TX_GPIO) { return F("TX0"); }
    if (gpio == RX_GPIO) { return F("RX0"); }
  ...
  if (includeSPI && Settings.isSPI_pin(gpio)) {
    int8_t spi_pins[3];
    if (Settings.getSPI_pins(spi_pins)) {
      if (spi_pins[0] == gpio) {
        return F("SPI SCLK");
      }
      else if (spi_pins[1] == gpio) {
        return F("SPI MISO");
      }
      else if (spi_pins[2] == gpio) {
        return F("SPI MOSI");
      }
    }
  }
