...
boolean Plugin_089(uint8_t function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_READ:
    {
      ...
      if ((WiFi.hostByName(hostname, remote_addr) == false) || (ping_start(remote_addr, 4, 0, 0, 5) == false)) {
        UserVar[event->BaseVarIndex]++;
        if (loglevelActiveFor(LOG_LEVEL_ERROR)) {
          String log = F("Hostname cannot be resolved or pinging: ");
          log += hostname;
      ...
      {
          String log = F("Hostname resolved and pinging success: ");
          log += hostname;
      ...
      P089_data_struct * P089_taskdata = static_cast<P089_data_struct *>(getPluginTaskData(event->TaskIndex));
